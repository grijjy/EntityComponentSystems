...
begin
  ...
  begin
    { We may update the position and velocity of the entity, so we need to get
      references to these }
    var PosRef := Entity.GetComponentRef<TPosition>;
    Assert(not PosRef.IsNil);

    var VelRef := Entity.GetComponentRef<TVelocity>;
    Assert(not VelRef.IsNil);

    { We don't need to update the dimensions, so we can just get its value
      (instead of a reference) }
    var Dim := Entity.GetComponent<TDimensions>;
...
begin
  ...
  begin
    ...
    var Pos := Entity.GetComponent<TPosition>;
    var Rot := Entity.GetComponent<TRotation>;
    var Dim := Entity.GetComponent<TDimensions>;
    var Mat := Entity.GetComponent<TMaterialIndex>;
