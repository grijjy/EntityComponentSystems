...
type
  ...
  { A light-weight entity.
    ...
    (see TWorld.AddComponent) and update those components using systems (see
    TSystem).
  ...
  { A specific TManager responsible for creating and destroying entities.
    ...
    methods of TWorld, which will in turn call into the entity manager. }
  TEntityManager = class(TManager)
  {$REGION 'Internal Declarations'}
  private
    { The value of the next entity that will be created. }
    FNext: TEntity;

    { A list of all entities currently alive. }
    FEntities: TList<TEntity>;

    { A list of all entities available for recycling. When an entity is
      destroyed, it is added to this list and reused the next time an entity
      is created. This ensures that most entities will stay grouped together
      and their values stay reasonably low (so the can be used as indices into
      other lists). }
    FEntitiesToRecycle: TList<TEntity>;
    ...
    FEntitiesToDestroy: TList<TEntity>;
  protected
  ...
  public
    ...
    { Destroys an entity.
      ...
      You usually don't call this method yourself, but use TWorld.DestroyEntity
      instead. }
  ...
  { A specific TManager that is reponsible for adding components to and removing
    ...
    methods of TWorld, which will in turn call into the component manager. }
  ...
  private
    ...
    { A list of component lists by component type.
      ...
      The index into that list is the the entity that owns that component.
  ...
  public
    ...
    { Get a reference to the component attached to the given entity.
      ...
      You usually don't call this method yourself, but use
      TWorld.GetComponentRef<T> instead. }
    ...
    { Removes a component from the given entity by type.
      ...
      You usually don't call this method yourself, but use
      TWorld.RemoveComponent<T> instead. }
  ...
  public
    ...
    { Adds a component to an entity.
      ...
      entities). }
    procedure AddComponent<T: record>(const AEntity: TEntity;
      const AComponent: T); inline;

    { Get the component attached to an entity by type.

      Parameters:
        T: the type of the component to retrieve.
        AEntity: the entity to retrieve the component from.
      ...
      If you need to update the component, then you should use GetComponentRef
      instead. }
    function GetComponent<T: record>(const AEntity: TEntity): T; inline;

    { Gets a reference to the component attached to an entity by type.
      ...
      If you don't need to update the component, then you should use
      GetComponent instead. }
    function GetComponentRef<T: record>(const AEntity: TEntity): TRef<T>; inline;
    ...
    { Removes a component from an entity by type.
      ...
      Raises:
        EAssertionFailed (if enabled) if the entity is unassigned or has been
        freed, or if there are more than 64 component types being used. }
    procedure RemoveComponent<T: record>(const AEntity: TEntity); inline;
...
begin
  ...
  begin
    ...
    begin
      var Entity := FEntitiesToDestroy[I];
      Assert(FEntities.Contains(Entity));
      FEntities.Remove(Entity);
      FEntitiesToRecycle.Add(Entity);
...
begin
  ...
  FEntitiesToRecycle := TList<TEntity>.Create;
  FEntitiesToDestroy := TList<TEntity>.Create;
end;
...
begin
  ...
  if (Index >= 0) then
  begin
    Result := FEntitiesToRecycle[Index];
    FEntitiesToRecycle.Delete(Index);
  end
  else
  begin
    { There are no recycled entities available. Generate a new one. }
    Result := AtomicIncrement(FNext);
  end;
...
begin
  ...
  Assert(FEntities.Contains(AEntity));
  Assert(not FEntitiesToDestroy.Contains(AEntity));
  FEntitiesToDestroy.Add(AEntity);
  AEntity := 0;
end;
...
begin
  ...
  { Get the list of all components of type T. The index in this list is the
    entity. Its value at this index is the component for that entity. }
  var Components := GetComponents<T>;

  Assert(AEntity < FEntityComponentBits.Count);
  Assert(AEntity < Components.Count);

  { Update the component for this entity }
  Components[AEntity] := AComponent;

  { Retrieve a bitset containing all components that are supported by this
    entity }
  var OldBits := FEntityComponentBits[AEntity];
  ...
  FEntityComponentBits[AEntity] := NewBits;
...
begin
  ...
  if (FEntityComponentBits.Count <= AEntity) then
    FEntityComponentBits.Count := AEntity + 1;

  { Clear any components associated with the entity }
  FEntityComponentBits[AEntity] := 0;
end;
...
begin
  ...
  Assert(AEntity < FEntityComponentBits.Count);
  if ((FEntityComponentBits[AEntity] and (1 shl TypeIndex)) = 0) then
    Exit(Default(T));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        { This entity supports at least the given combination of components.
          Add it to the list. }
        Result.Add(I);
      end;
    end;
...
begin
  ...
  Info.Entity := AEntity;
  Info.OldBits := FEntityComponentBits[AEntity];
  Info.NewBits := Info.OldBits and not (1 shl TypeIndex);
  FEntityComponentBits[AEntity] := Info.NewBits;
...
procedure TWorld.AddComponent<T>(const AEntity: TEntity; const AComponent: T);
begin
  FComponentManager.AddComponent(AEntity, AComponent);
end;
...
function TWorld.GetComponent<T>(const AEntity: TEntity): T;
begin
  Result := FComponentManager.GetComponent<T>(AEntity);
end;

function TWorld.GetComponentRef<T>(const AEntity: TEntity): TRef<T>;
begin
  Result := FComponentManager.GetComponentRef<T>(AEntity);
end;
...
procedure TWorld.RemoveComponent<T>(const AEntity: TEntity);
begin
  FComponentManager.RemoveComponent<T>(AEntity);
end;
